...
### Multi-Container Orchestration (The Fleet)
- **Infrastructure as Code (IaC)**: Authored a `docker-compose.yml` blueprint to define and manage a multi-service web environment as a single unit.
- **Service Orchestration**: Successfully synchronized a custom-built **Admin Dashboard** (Port 8081) and an **Nginx Test Server** (Port 8080) to run concurrently.
- **Unified Lifecycle Management**: Mastered the `docker compose` workflow, using a single command (`up -d`) to build images, create networks, and launch services.
- **Environment Scalability**: Implemented a "Set it and Forget it" configuration that allows the entire lab environment to be replicated on any Docker-enabled host in seconds.

### 🛠️ Updated Tech Stack & Workflow
